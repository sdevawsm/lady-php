...
class QueryBuilder
{
    ...
    public function get(): array
    {
        ...
        // Debug para ver a SQL e os bindings
        var_dump([
            'sql' => $sql,
            'bindings' => $bindings
        ]);
        ...
        if (!empty($this->wheres)) {
            ...
            foreach ($this->wheres as $where) {
                if ($where['operator'] === 'IN') {
                    ...
                    $condition = "{$where['column']} IN ({$placeholders})";
                } else {
    ...
    /**
     * Retorna os valores para binding
     */
        ...
        foreach ($this->wheres as $where) {
            if ($where['operator'] === 'IN') {
                foreach ($where['value'] as $value) {
                    $bindings[] = $this->castBindingValue($value);
                }
            } else {
                $bindings[] = $this->castBindingValue($where['value']);
            }
        }
        return $bindings;
    }
    ...
    /**
     * Converte o valor para o tipo apropriado para binding
     */
    protected function castBindingValue($value)
    {
        if (is_numeric($value)) {
            return (int) $value;
        }
    ...
    {
        // TODO: Implementar contagem de relacionamentos
    }
} 